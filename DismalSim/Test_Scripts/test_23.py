...
The script constructs a graph involving only NIR and not RIR, and runs
it through the time period of 1990-1999; the regressions used to
calculate edge relationships were performed on the same time period, in
order to test the fundamental robustness of the model. This test script
involves the correct transmittances to FX.
"""
...
aGraph + digraph.Vertex("C", 3825.6, deltaInherent=38.0)
...
aGraph + digraph.Vertex("PL", 66.77, deltaInherent=-4.94)
aGraph + digraph.Vertex("I", 993.5, deltaInherent=58.6)
aGraph + digraph.Vertex("FX", 71.41, deltaInherent=-3.10)
...
aGraph.add_edge("YD", "C", "aa_lin", [0.940])
aGraph.add_edge("NIR", "C", "aa_lin", [-4.16])
...
aGraph.add_edge("NIR", "I", "aa_lin", [3.20])
aGraph.add_edge("Y", "I", "aa_lin", [0.151])
aGraph.add_edge("NIR", "FX", "aa_lin", [-2.93])
aGraph.add_edge("EX", "FX", "aa_lin", [0.0409])
aGraph.add_edge("IM", "FX", "aa_lin", [0.119])
...
deltacalc.output_spreadsheet("test_23_data", output)