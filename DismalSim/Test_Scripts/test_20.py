...
The script constructs a modified graph corresponding to the version of
the economic model in which G is exogenous and increases by a given
percent per year. It runs it through the time period of 1990-1999; the
regressions used to calculate edge relationships were performed on the
same time period, in order to test the fundamental robustness of the
model. This test script involves the correct transmittances to FX.
"""

aGraph = digraph.DiGraph()
aGraph + digraph.Vertex("G", 1920.2, deltaInherent=4.43, percentFlag=True)
aGraph + digraph.Vertex("T", 1712.9, deltaInherent=61.4)
...
aGraph.add_edge("G", "T", "aa_lin", [0.857])
...
aGraph.add_edge("RIR", "FX", "aa_lin", [-3.21])
aGraph.add_edge("EX", "FX", "aa_lin", [0.0260])
aGraph.add_edge("IM", "FX", "aa_lin", [0.116])
